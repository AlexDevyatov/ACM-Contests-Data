...
pair<int, double> where(vector<Point<double>>& a, double length) {
    double distance = 0;
    rep(i,0,a.size() - 1) {
        if (distance + (a[i] - a[i + 1]).dist() > length)
            return make_pair(i, length - distance);
        distance += (a[i] - a[i + 1]).dist();
    }
    return make_pair(a.size(), -1);
}
...
double closest(vector<Point<double>>& a, vector<Point<double>>& b) {
    ...
    double distance = 0;
    rep(i,0,a.size() - 1) {
        pair<int, double> position = where(b, distance);
