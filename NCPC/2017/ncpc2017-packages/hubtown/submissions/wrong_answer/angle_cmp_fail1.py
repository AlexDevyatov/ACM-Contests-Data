...
def find_closest(P, A, B):
    dotA = dot(P, A)
    dotB = dot(P, B)
    if sgn(dotA) != sgn(dotB):
        return [0] if sgn(dotA) > sgn(dotB) else [1]
    num = dotA**2 * dot(B, B)
    den = dotB**2 * dot(A, A)
    if num == den: return [1, 0]
    return [0] if (num > den) else [1]
    # Ooops, correct is:
    #    return [0] if (num > den) ^ (dotA < 0) else [1]
    # Fails to correctly find closest point to P when both are > 90 degrees away and at different dist
