...
int main(void) {
    ...
    fprintf(stderr, "%d possible opts\n", goals.size());
    int tried = 0;
    // Ooops linear scan through possible opt values instead of binary search
    // tries to be clever by breaking on first found.
    for (auto goal: goals) {
        ++tried;
        if (check(n, s, types, c, goal)) {
            fprintf(stderr, "%d'th cand worked\n", tried);
            printf("%d\n", goal);
            break;
        }
    }
