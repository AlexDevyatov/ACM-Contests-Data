...
void insertInto2(cart n, deque<cart> &q, double var) {
    q.clear();
    q.push_front(n);
}
...
#define DOINTERVAL2(idx, queue, var, left, right) \
        while (P[idx] > left) { \
            insertInto2(make_tuple(P[idx], opt[idx], idx), queue, var); \
            ++idx; \
        } \
        while (!queue.empty() && get<0>(queue.back()) > right) { \
            queue.pop_back(); \
        ...
        DOINTERVAL2(drhas, drinking, b, waitingEndPos, drinkingEndPos);
