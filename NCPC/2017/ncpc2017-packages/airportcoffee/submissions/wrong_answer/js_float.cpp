...
typedef tuple<float, float, int> cart;

void insertInto(cart n, deque<cart> &q, float var) {
    auto cmp = [var](cart x) {
...
int main() {
    ...
    vi optNext(sz(P));
    optNext[0] = -1;
    vector<float> opt(sz(P));
    deque<tuple<float, float, int>> waiting, drinking, done;
    int whas = 0, drhas = 0, dohas = 0;
    rep(i,1,sz(P)) {
        float pos = P[i];
        // Pop away everything in the queues that are too far away

        float waitingEndPos = pos + t * a;
        float drinkingEndPos = pos + t * a + r * b;
...
#define SMIN(idx) if (nans < ans) { ans = nans; ansi = idx; }
        if (!waiting.empty()) {
            float nans = (get<0>(waiting.back()) - pos) / a + get<1>(waiting.back());
            SMIN(get<2>(waiting.back()));
        }
        if (!drinking.empty()) {
            float nans = t + (get<0>(drinking.back()) - pos - t * a) / b + get<1>(drinking.back());
            SMIN(get<2>(drinking.back()));
        }
        if (!done.empty()) {
            float nans = t + r + (get<0>(done.back()) - pos - t * a - r * b) / a + get<1>(done.back());
