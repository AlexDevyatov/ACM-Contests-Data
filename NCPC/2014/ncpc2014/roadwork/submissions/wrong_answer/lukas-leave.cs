...
class Problem
{
    ...
    public static void Main ()
    {
        ...
        {
            int nproc = (proc + 1) % 2;
            for (int east = 0; east <= q[0].Count; east++)
                    ...
                    for (int side = 0; side < 2; side++)
            {
                ...
                foreach (int nside in Enumerable.Range(0, 2).Where(z => c[z] < q[z].Count))
                {
                    ...
                    if (nside == side)
                        ntime = Math.Max(time[proc % 2, east, irr, side] + 3, ntime);
                    else
                        ntime = Math.Max(time[proc % 2, east, irr, side] + t, ntime);

                    if (ntime > w.Item2 + w.Item1 + t)
                    {
                        nirr++;
                        ntime = time[proc % 2, east, irr, side];
                    }
