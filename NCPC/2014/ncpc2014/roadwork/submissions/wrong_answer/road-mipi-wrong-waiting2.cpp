/* Wrong transitions in DP: +3s is counted from the max(can go, arrival), 
   so if the arrival is later than the moment a car can go, then it still 
   needs to wait 3s. */
...
//		printf("State (%d,%d,%d,%d), message: %s %d\n",status,west,east,irritated,msg,r);
	...
	void computeValue() const {
		...
		if (status==WEST){
			...
			if (irritated>0){
				cnd = max(T[west-1][east][irritated-1][WEST],arrival[WEST][west-1])+time_to_wait;
				printSelf("1.a",cnd);
				bst = min(bst,cnd);
			}

			/* Case 1.b: the current car did not get irritated */
			cnd = max(T[west-1][east][irritated][WEST],arrival[WEST][west-1])+time_to_wait;
		...
		if (status==EAST){
			...
			/* Case 1.a: the current car could have got irritated */
			if (irritated>0){
				cnd = max(T[west][east-1][irritated-1][EAST],arrival[EAST][east-1])+time_to_wait;
				printSelf("1.a",cnd);
				bst = min(bst,cnd);
			}

			/* Case 1.b: the current car did not get irritated */
			cnd = max(T[west][east-1][irritated][EAST],arrival[EAST][east-1])+time_to_wait;
