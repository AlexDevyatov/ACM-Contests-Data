...
public class SquMipi {
  ...
  public static long countSquares(ArrayList<Long> r1,ArrayList<Long> r2){
	long[] p1 = new long[(r1.size()*(r1.size()-1))/2];
	long[] p2 = new long[(r2.size()*(r2.size()-1))/2];

	int u=0;
	for(int j=0;j<r1.size();j++) for(int i=0;i<j;i++) p1[u++]=r1.get(j)-r1.get(i);
	    u=0;	
	for(int j=0;j<r2.size();j++) for(int i=0;i<j;i++) p2[u++]=r2.get(j)-r2.get(i);
