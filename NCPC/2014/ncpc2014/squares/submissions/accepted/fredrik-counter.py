...
linesdiffs = {}  # normal: Counter(distance_difference)
for n, dists in lines.items():
    ...
    linesdiffs[n] = Counter(distdiffs)

# Count...
ret = 0
for n, ndistdiffs in linesdiffs.items():
    p = rotateNormal((n[1], -n[0]))  # Perpendicular to the normal
    pdistdiffs = linesdiffs.get(p, Counter())
    commondiffs = ndistdiffs & pdistdiffs  # At least 1 in each counter
    for distdiff in commondiffs.keys():
        ncount = ndistdiffs[distdiff]
        pcount = pdistdiffs[distdiff]
        ret += ncount * pcount
